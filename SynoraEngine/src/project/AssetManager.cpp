...
void SYN::AssetManager::addRef(UUID id) {
  if (m_AssetMap.contains(id)) {
    ...
    m_PendingRefChanges[id]++;
  } else if (id != 0){
    spdlog::warn("Asset Manager: Could not add ref {}, asset missing", id);
  }
}

void SYN::AssetManager::flushRefCount() {
  for (auto [id, delta] : m_PendingRefChanges) {
    assert(m_AssetMap.contains(id));

    auto asset = m_AssetMap[id].data;
    int64_t ref = m_AssetMap[id].ref + delta;
    if(m_AssetMap[id].ref == 0 && ref > 0) {
    ...
    } else if (m_AssetMap[id].ref > 0 && ref < 0) {
      if (std::holds_alternative<MeshData>(asset)) {
    ...
    m_AssetMap[id].ref = ref;
    m_AssetMap[id].ref = std::max(m_AssetMap[id].ref, 0l);
  }
  m_PendingRefChanges.clear();
}
